...
class AuthViewSet(viewsets.GenericViewSet):
    ...
    @action(
        detail=False, methods=["post"], serializer_class=ConfirmRegistrationSerializer
    )
    ...
    @action(
        detail=False, methods=["post"], serializer_class=RequestOnboardingLinkSerializer
    )
    def request_onboarding_link(self, request):
        ...
        success, message = send_onboarding_notification(
            user, raw_password=new_raw_password
        )
        if not success:
            raise serializers.ValidationError(
                {"error": _("Failed to send link."), "details": message}
            )
    ...
    @action(
        detail=False, serializer_class=ForgotPasswordConfirmSerializer, methods=["post"]
    )
    ...
    def user_profile_picture(self, request):
        ...
        if request.method.lower() == "get":
            ...
            serializer = self.get_serializer(
                instance=user, data=request.data, partial=is_partial
            )
            serializer.is_valid(raise_exception=True)
            serializer.save()
    ...
    def user_addresses(self, request):
        ...
        if request.method.lower() == "get":
            ...
            serializer = self.get_serializer(
                address, data=request.data, partial=(request.method.lower() == "patch")
            )
            serializer.is_valid(raise_exception=True)
            serializer.save()
            return Response(
                {"detail": "Address updated successfully."}, status=status.HTTP_200_OK
            )
    ...
    def user_preferences(self, request):
        ...
        if request.method.lower() == "get":
            try:
                ...
                return Response(
                    {"error": "Preferences not set."}, status=status.HTTP_404_NOT_FOUND
                )

        elif request.method.lower() == "post":
            if hasattr(user, "preferences"):
                return Response(
                    {"error": "Preference already exist."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            serializer = self.get_serializer(data=request.data)
            serializer.is_valid(raise_exception=True)
            ...
            return Response(
                {
                    ...
                    "data": {
                        ...
                        "phone_number": (
                            str(employee.user.phone_number)
    ...
    def confirm_staff_onboarding(self, request):
        ...
        try:
            ...
            raise serializers.ValidationError(
                {
                    "error": _(
                        "Invalid user identifier. Please check the link or contact support."
                    )
                }
            )
        ...
        if not default_token_generator.check_token(user, token):
            raise serializers.ValidationError(
                {
                    "error": _(
                        "The setup link has expired or is invalid. Please request a new one."
                    )
                }
            )
        ...
        with transaction.atomic():
            ...
            verification_history.append(
                {"event": "Account activated", "timestamp": now_iso, "by": user_str}
            )
        ...
        return Response(
            {
                "message": _(
                    "Account setup complete. You can now sign in to the dashboard."
                ),
            },
            status=status.HTTP_200_OK,
        )
