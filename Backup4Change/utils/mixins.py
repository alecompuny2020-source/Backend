...
class DispatcherService:
    """
    Handles the Bolt-style logic of matching riders to the nearest
    available 'Vyombo vya Moto'.
    """

    @staticmethod
    def get_closest_drivers(pickup_lat, pickup_lng, requested_vehicle_type, limit=5):
        """
        Executes a geospatial proximity search on Driver telemetry.
        """
        query = """
            SELECT
                d.driver_id,
                ...
                SQRT(
                    POWER((d.current_telemetry->>'lat')::float - %s, 2) +
                    POWER((d.current_telemetry->>'lng')::float - %s, 2)
                ) AS distance_score
            FROM
                driver d
            JOIN
                vehicle v ON d.driver_id = v.owner_id
            JOIN
                vehicle_model vm ON v.model_id = vm.model_id
            WHERE
                d.is_available = true
        ...
        # Use the specific mobility database vault
        with connections["amenities_db"].cursor() as cursor:
            cursor.execute(
                query, [pickup_lat, pickup_lng, requested_vehicle_type, limit]
            )

            # Map results to a readable dictionary for the Angular frontend
