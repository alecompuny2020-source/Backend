...
class CropsManagementService:
    ...
    @staticmethod
    @transaction.atomic
    def process_harvest_to_feed_stock(
        crop_production_id: int,
        actual_yield: Decimal,
        unit_cost_tzs: Decimal = Decimal("0.00"),
    ) -> FeedIngredientStock:
        """
        ...
        if actual_yield <= 0:
            raise ValidationError(
                "Kiwango cha mavuno (actual_yield) lazima kiwe zaidi ya sifuri."
            )

        try:
            production = CropProduction.objects.select_for_update().get(
                id=crop_production_id
            )
        except CropProduction.DoesNotExist:
            raise ValidationError(
                f"Uzalishaji wa zao wenye ID {crop_production_id} haujapatikana."
            )

        if production.status == CropProduction.ProductionStatus.HARVESTED:
            raise ValidationError(
                "Zao hili tayari lilikwisha wekwa kwenye hali ya 'Imevunwa'."
            )
        ...
        production.production_metadata["processed_to_feed_at"] = (
            timezone.now().isoformat()
        )
        production.production_metadata["financial_value_tzs"] = float(
            actual_yield * unit_cost_tzs
        )
        production.save()
        ...
        (
            ingredient_stock,
            created,
        ) = FeedIngredientStock.objects.select_for_update().get_or_create(
        ...
        if not created and unit_cost_tzs > 0:
            total_existing_value = (
                ingredient_stock.available_qty_kg * ingredient_stock.unit_cost_per_kg
            )
            total_new_value = actual_yield * unit_cost_tzs
            total_qty = ingredient_stock.available_qty_kg + actual_yield

            if total_qty > 0:
                ingredient_stock.unit_cost_per_kg = (
