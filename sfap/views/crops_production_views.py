from sfap.serializers import CropProductionSerializer, LogCropHarvestSerializer, EcologicalInputSerializer, FeedIngredientStockSerializer
...
class CropProductionViewSet(EnterpriseObjectLevelPermissionMixin, BaseEnterpriseViewSet):
    """
    ...
    @action(detail=True, methods = ['post'], serializer_class = LogCropHarvestSerializer, url_path = 'log-harvest')
    def log_harvest(self, request, pk=None):
        ...
        try:
            ...
            return Response(_("Mavuno yamefanikiwa kurekodiwa na stoki ya chakula imeongezwa."))
...
class EcologicalInputViewSet(EnterpriseObjectLevelPermissionMixin, BaseEnterpriseViewSet):
    queryset = EcologicalInput.objects.select_related('origin').all()
    serializer_class = EcologicalInputSerializer

    
class FeedIngredientStockViewSet(EnterpriseObjectLevelPermissionMixin, viewsets.ReadOnlyModelViewSet):
